...
**问题**：项目名包含 "某直聘" 商标
**解决**：
- [x] 对外名称改为 `OfferCatch`（中性名称）
- [x] README 免责声明放在最顶部
- [x] LICENSE 追加 ADDITIONAL DISCLAIMER
- [x] 代码中移除 "offercatch" → 改为 "offercatch"（包名、模块名）
...
### 代码层面
- [x] 全局替换 `offercatch` → `offercatch`（包名、import、CLI 名）
...
### 前端层面
- [ ] 构建 production 版本：`cd src/offercatch/web/frontend && npm run build`
...
| 原名 | 新名 | 位置 |
|------|------|------|
| offercatch | offercatch | Python 包名 |
| OfferCatch | OfferCatch | 项目显示名 |
| offercatch.db | offercatch.db | 数据库文件名 |
| `offercatch` CLI | `offercatch` CLI | 命令行入口 |
| src/offercatch/ | src/offercatch/ | 源码目录 |
