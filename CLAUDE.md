...
输出以上内容后，执行 `offercatch web` 启动面板。
...
当用户要求"启动"、"开始"、"run"、"投递"时，**不要直接执行 `offercatch run`**，必须先完成以下检查：
...
>
> 请先完成简历上传：
> - 打开配置面板：`offercatch web` → http://127.0.0.1:8686
...
**只有确认简历是用户本人的真实简历后，才可以执行 `offercatch run`。**
...
```bash
offercatch web              # 打开 Web 配置面板
offercatch connect          # 检测 Chrome CDP 连接
offercatch ai-status        # 安全检测 AI API 连接（不显示 Key）
offercatch run              # 一键执行完整流程
offercatch scrape -k "关键词"  # 采集岗位
offercatch score            # AI 评分
offercatch greet            # 生成招呼语
offercatch confirm          # 人工确认
offercatch send             # 发送
offercatch monitor          # 监听 HR 回复
offercatch status --full    # 查看状态
```
