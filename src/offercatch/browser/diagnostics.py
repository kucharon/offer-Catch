...
from offercatch.browser import find_boss_tab
from offercatch.browser.runtime import check_node_available, ensure_runtime, get_runtime_url, runtime_health, runtime_targets
...
def run_browser_diagnostics(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not node.get("available") and not runtime_ready:
        errors.append("Node.js is required for OfferCatch Browser Runtime.")
    if not runtime_ready:
        if health and health.get("runtime") != "offercatch":
            errors.append("Runtime port is occupied by a non-OfferCatch service.")
        else:
            errors.append("OfferCatch Browser Runtime is not ready.")
...
def print_browser_diagnostics(config: dict[str, Any] | None = None, console: Console | None = None) -> bool:
    ...
    if node.get("available"):
        ...
        out.print("  OfferCatch 内置浏览器运行时需要 Node.js。请安装 Node.js 22+ 后重试。")

    if result["runtime"]:
        out.print(f"[green]✓[/green] OfferCatch CDP Runtime 已启动: {result['runtime_url']}")
    else:
        out.print(f"[red]✗[/red] OfferCatch CDP Runtime 未就绪: {result['runtime_url']}")
        if any("non-OfferCatch" in error for error in result["errors"]):
            out.print("  端口已被非 OfferCatch 服务占用，请停止旧的 CDP Proxy 或在 config.yaml 中修改 browser.proxy_port。")

    if result["chrome"]:
        out.print("[green]✓[/green] 已连接 Chrome")
    elif any("non-OfferCatch" in error for error in result["errors"]):
