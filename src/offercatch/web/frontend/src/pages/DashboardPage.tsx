...
async function parsePreflightResponse(res: Response) {
  ...
  try {
    ...
    return {
      ok: false,
      messages: [message],
      checks: [{ id: 'preflight_api', title: '启动检查', status: 'error', message, detail: '请重启 OfferCatch 后重试。' }] as PreflightCheck[],
    }
  }
...
export default function DashboardPage({ view = 'workbench' }: DashboardPageProps) {
  ...
  const retryPreflight = async () => {
    ...
    try {
      ...
      setNotice('重新检查失败，请确认 OfferCatch 后端仍在运行。')
    } finally {
      setModePending(null)
    }
  }
...
function JobsPoolView() {
  ...
  const exportJobs = async (format: 'xlsx' | 'csv', scope: 'all' | 'filtered' | 'selected') => {
    try {
      ...
      const filename = disposition.match(/filename="?([^";]+)"?/i)?.[1] || `offercatch-jobs.${format}`
