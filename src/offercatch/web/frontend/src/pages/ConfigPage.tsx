...
export default function ConfigPage() {
  ...
  const handleAiTest = async () => {
    ...
    try {
      ...
      setAiTest({ testing: false, ok: false, message: '无法连接本地检测接口，请确认 OfferCatch 后端正在运行。' })
    }
  }
      ...
      <div className="flex h-full items-center justify-center">
        <div className="max-w-md rounded-2xl border border-card-border bg-[#FFFCFA] p-6 text-center">
          ...
          <p className="mt-2 text-xs leading-6 text-muted">
            请确认后端服务已启动：在项目根目录运行 offercatch web，或启动 127.0.0.1:8686 后刷新页面。
          </p>
          ...
          <div className="space-y-4">
            <Field label="提供商">
              ...
              <p className="mt-1 text-xs text-muted">
                OfferCatch 会自动配置协议和服务地址；也可安全复用环境变量 {
