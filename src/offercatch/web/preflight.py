...
from offercatch.ai.credentials import (
	get_ai_api_key,
	get_ai_base_url,
	get_ai_key_source,
	get_ai_service,
	...
	if result.status_code in {404, 405}:
		return [
			_check(
				...
				"warning",
				"接口可访问，但无法自动验证模型",
				"该兼容服务未提供模型列表接口；OfferCatch 启动时仍会尝试消息接口。",
				"config",
			)
		]
	...
	if result.get("runtime"):
		...
		checks.append(
			_check(
				...
				"error",
				"缺少可用的 Node.js",
				"OfferCatch 无法启动浏览器连接组件，请安装 Node.js 22+ 后重启。",
				"browser",
			)
		)
	else:
		if any("non-OfferCatch" in error for error in errors):
			message = "浏览器连接端口被其他程序占用"
			detail = "请停止占用本地 Runtime 端口的程序，或修改 browser.proxy_port 后重启。"
		else:
			message = "OfferCatch 浏览器运行组件未启动"
			detail = "请重新启动 OfferCatch；若仍失败，运行 offercatch connect 查看终端诊断。"
