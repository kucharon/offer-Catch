"""OfferCatch Web Server - Bottle HTTP service + API routes.
...
from offercatch import __version__
from offercatch.ai.credentials import get_ai_api_key
from offercatch.cities import CityRefreshError, get_city_map, load_city_snapshot, refresh_city_cache
from offercatch.config import AI_SERVICE_PRESETS, load_config
...
from offercatch.job_filters import parse_monthly_salary_k
from offercatch.job_export import InvalidJobSelectionError, export_jobs, export_row_count
from offercatch.scoring_run_store import (
	create_scoring_run,
	get_scoring_run,
	list_scoring_runs,
...
from offercatch.scoring_selection import preview_scoring, select_scoring_jobs, validate_options
from offercatch.web.preflight import check_ai_connection, collect_preflight_checks, error_messages
from offercatch.web.resume_upload import ResumeUploadError, prepare_resume_content
from offercatch.web.city_lookup import CityLookupError, lookup_city
from offercatch.web.tasks import (
...
def set_base_dir(base_dir: Path | str) -> None:
	...
	RESUME_DIR = DATA_DIR / "resumes"
	CONFIG_PATH = BASE_DIR / "config.yaml"
	mark_orphaned_scoring_runs_paused(DATA_DIR / "offercatch.db")


def _get_web_db():
	"""Open the dashboard database from the resolved runtime data directory."""
	return get_db(DATA_DIR / "offercatch.db")
...
def _execute_collect(task: WorkbenchTask, config: dict) -> None:
	from offercatch.ai.scorer import score_jobs
	from offercatch.scraper.jobs import scrape_jobs
...
def _execute_score(task: WorkbenchTask, config: dict) -> None:
	...
	db_path = DATA_DIR / "offercatch.db"
...
def _execute_monitor(task: WorkbenchTask, config: dict, *, initial_cooldown: bool = False) -> None:
	from offercatch.executor.monitor import monitor_and_send_resumes
...
def _execute_deliver_batch(task: WorkbenchTask, config: dict) -> None:
	from offercatch.ai.greeter import generate_greetings
...
def api_scoring_runs():
	return _json_response(list_scoring_runs(DATA_DIR / "offercatch.db"))
...
def api_history_reply(history_id):
	...
	try:
		...
		from offercatch.executor.monitor import _build_reply_resolution_detail
