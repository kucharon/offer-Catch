...
from offercatch.browser import (
    new_tab,
    close_tab,
    evaluate,
    click_at,
...
from offercatch.db import get_db, get_jobs_ready_to_send, update_job_status, add_history, add_risk_event
from offercatch.throttle import RequestThrottle, SendWindowChecker, ProgressiveBackoff, should_take_day_off
...
def _submit_chat_message_background(target_id: str, greeting: str) -> dict:
    """Use OfferCatch's original Vue submit path without foregrounding Chrome."""
    greeting_escaped = json.dumps(greeting, ensure_ascii=False)
    result = _parse_js_result(evaluate(target_id, f"""
    (() => {{
        const input = document.querySelector('#chat-input');
...
def send_greetings(config: dict, force: bool = False) -> int:
    ...
    if not jobs:
        console.print("[yellow]没有已生成招呼语的待发送岗位，请先运行 offercatch greet[/yellow]")
