...
from offercatch.ai import credentials, greeter, scorer
...
class AiCredentialErrorTests(unittest.TestCase):
    def test_openai_compatible_quota_error_is_not_silently_swallowed(self):
        ...
        with patch("offercatch.ai.credentials.httpx.post", return_value=response):
...
class ScorerTokenResilienceTests(unittest.TestCase):
    ...
    def test_borderline_structured_score_is_reviewed_and_averaged(self):
        ...
        with (
            patch("offercatch.ai.scorer.get_db", return_value=db),
            patch("offercatch.ai.scorer._load_resume", return_value="真实简历"),
            patch("offercatch.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("offercatch.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("offercatch.ai.scorer._call_claude", side_effect=[first, review]) as call_ai,
            patch("offercatch.ai.scorer.update_job_quick_score"),
            patch("offercatch.ai.scorer.update_job_score") as update_score,
            patch("offercatch.ai.scorer.update_job_status"),
        ):
    ...
    def test_ai_calls_run_with_configured_concurrency_but_db_writes_stay_on_main_thread(self):
        ...
        with (
            patch("offercatch.ai.scorer.get_db", return_value=db),
            patch("offercatch.ai.scorer._load_resume", return_value="真实简历"),
            patch("offercatch.ai.scorer.get_jobs_by_status", return_value=jobs),
            patch("offercatch.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("offercatch.ai.scorer._call_claude", side_effect=call_ai),
            patch("offercatch.ai.scorer.update_job_quick_score", side_effect=record_write),
            patch("offercatch.ai.scorer.update_job_score", side_effect=record_write),
            patch("offercatch.ai.scorer.update_job_status", side_effect=record_write),
        ):
            scored, filtered = scorer.score_jobs(
    ...
    def test_stop_returns_without_waiting_for_inflight_concurrent_ai_calls(self):
        ...
        with (
            patch("offercatch.ai.scorer.get_db", return_value=db),
            patch("offercatch.ai.scorer._load_resume", return_value="真实简历"),
            patch("offercatch.ai.scorer.get_jobs_by_status", return_value=[_job("1"), _job("2")]),
            patch("offercatch.ai.scorer.quick_score", return_value=(80, "通过")),
            patch("offercatch.ai.scorer._call_claude", side_effect=blocking_ai),
            patch("offercatch.ai.scorer.update_job_quick_score"),
            patch("offercatch.ai.scorer.update_job_score"),
            patch("offercatch.ai.scorer.update_job_status"),
        ):
    ...
    def test_scoring_processes_all_unscored_pending_jobs(self):
        ...
        with (
            patch("offercatch.ai.scorer.get_db", return_value=db),
            patch("offercatch.ai.scorer._load_resume", return_value="真实简历"),
            patch("offercatch.ai.scorer.get_jobs_by_status", return_value=[old_job, new_job]),
            patch("offercatch.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "offercatch.ai.scorer._call_claude",
                return_value=_score_response(82),
            ) as call_ai,
            patch("offercatch.ai.scorer.update_job_quick_score") as update_quick_score,
            patch("offercatch.ai.scorer.update_job_score"),
            patch("offercatch.ai.scorer.update_job_status"),
        ):
            scored, filtered = scorer.score_jobs(
    ...
    def test_context_limit_retries_once_with_compact_prompt(self):
        ...
        with (
            patch("offercatch.ai.scorer.get_db", return_value=db),
            patch("offercatch.ai.scorer._load_resume", return_value="简历内容" * 1000),
            patch("offercatch.ai.scorer.get_jobs_by_status", return_value=[job]),
            patch("offercatch.ai.scorer.quick_score", return_value=(80, "通过")),
            patch(
                "offercatch.ai.scorer._call_claude",
    ...
    def test_quota_error_pauses_without_changing_pending_jobs(self):
        ...
        with (
            ...
            patch("offercatch.ai.scorer.update_job_quick_score"),
            patch("offercatch.ai.scorer.update_job_score"),
            patch("offercatch.ai.scorer.update_job_status") as update_status,
        ):
            scored, filtered = scorer.score_jobs(
                {
                    "scoring": {"threshold": 70},
...
class GreeterTokenResilienceTests(unittest.TestCase):
    ...
    def test_invalid_review_format_keeps_the_generated_greeting(self):
        ...
        with (
            patch("offercatch.ai.greeter.get_db", return_value=db),
            patch("offercatch.ai.greeter.get_jobs_by_status", return_value=jobs),
            patch("offercatch.ai.greeter._get_resume_summary", return_value="真实简历摘要"),
            patch(
                "offercatch.ai.greeter._call_claude",
                side_effect=[
                    "这是一条可用的个性化招呼语。",
                    "评分很好，但没有按 JSON 返回。",
                ],
            ) as call_ai,
            patch("offercatch.ai.greeter.update_job_greeting") as update_greeting,
            patch("offercatch.ai.greeter.update_job_status") as update_status,
        ):
    ...
    def test_empty_greeting_retries_before_leaving_job_pending(self):
        ...
        with (
            ...
            patch("offercatch.ai.greeter.update_job_greeting") as update_greeting,
            patch("offercatch.ai.greeter.update_job_status"),
            patch("offercatch.ai.greeter.add_history") as add_history,
        ):
            count = greeter.generate_greetings(
                {
                    "ai": {
    ...
    def test_truncated_greeting_retries_with_larger_output_limit(self):
        ...
        with (
            ...
            patch("offercatch.ai.greeter.update_job_greeting"),
            patch("offercatch.ai.greeter.update_job_status"),
        ):
            count = greeter.generate_greetings(
                {
