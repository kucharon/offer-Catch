...
from offercatch.throttle import SendWindowChecker
from offercatch.web import server
from threading import Event, Lock

from offercatch.web.tasks import TaskAlreadyRunningError, WorkbenchTask, WorkbenchTaskRunner
...
class WebApiRouteTests(unittest.TestCase):
    ...
    def _upload_resume(self, filename: str, content: bytes, content_type: str):
        boundary = "----OfferCatchResumeUpload"
        body = (
            (
                f'--{boundary}\r\n'
    ...
    def test_web_api_workbench_preflight_full_requires_ai_key(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            browser_ready = {
                ...
                "health": {"runtime": "offercatch"},
                "browser_product": "Chrome/138.0",
            }
            with (
                patch.dict("os.environ", {}, clear=True),
                patch("offercatch.web.preflight.run_browser_diagnostics", return_value=browser_ready),
            ):
                status, headers, body = self._request("/api/workbench/preflight?mode=full")
    ...
    def test_job_search_filters_keyword_score_salary_and_status(self):
        with tempfile.TemporaryDirectory() as tmp:
            base_dir = Path(tmp)
            db = get_db(base_dir / "data" / "offercatch.db")
            try:
    ...
    def test_task_runner_automatically_stops_at_send_window_deadline(self):
        ...
        # Act
        with patch("offercatch.web.tasks._deadline_from_config", return_value=deadline):
    ...
    def test_web_api_deliver_queues_confirmation_before_full_task_event_exists(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            verify_db = get_db(base_dir / "data" / "offercatch.db")
            try:
    ...
    def test_monitor_loop_processes_queued_delivery_before_next_check(self):
        ...
        with patch.object(server, "_execute_deliver") as execute_deliver, \
             patch(
                 "offercatch.executor.monitor.monitor_and_send_resumes",
    ...
    def test_deliver_keeps_partial_result_and_continues_after_single_failure(self):
        ...
        with patch("offercatch.ai.greeter.generate_greetings", return_value=3), \
             patch("offercatch.executor.sender.send_greetings", side_effect=fake_send):
            server._execute_deliver(task, config)
    ...
    def test_deliver_still_stops_on_account_risk_signal(self):
        ...
        # Act / Assert
        with patch("offercatch.ai.greeter.generate_greetings", return_value=2), \
             patch("offercatch.executor.sender.send_greetings", side_effect=fake_send), \
