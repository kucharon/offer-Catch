...
class BrowserDiagnosticsTests(unittest.TestCase):
    @patch("offercatch.browser.diagnostics.httpx.get")
    def test_browser_identity_detects_edge_from_older_runtime_health(self, http_get):
        from offercatch.browser.diagnostics import _browser_identity
    ...
    @patch("offercatch.browser.diagnostics.find_boss_tab")
    @patch("offercatch.browser.diagnostics.runtime_targets")
    @patch("offercatch.browser.diagnostics.ensure_runtime")
    @patch("offercatch.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_ready_runtime(self, check_node, ensure_runtime, runtime_targets, find_boss_tab):
        from offercatch.browser.diagnostics import run_browser_diagnostics
    ...
    @patch("offercatch.browser.diagnostics.find_boss_tab")
    @patch("offercatch.browser.diagnostics.runtime_targets")
    @patch("offercatch.browser.diagnostics.runtime_health")
    @patch("offercatch.browser.diagnostics.ensure_runtime")
    @patch("offercatch.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_updated_url_after_runtime_port_fallback(
        ...
        runtime_health.return_value = {"status": "ok", "runtime": "offercatch"}
        runtime_targets.return_value = [{"targetId": "1", "url": "https://www.zhipin.com"}]
        find_boss_tab.return_value = {"targetId": "1", "title": "BOSS直聘"}
    ...
    @patch("offercatch.browser.diagnostics.ensure_runtime")
    @patch("offercatch.browser.diagnostics.check_node_available")
    def test_run_browser_diagnostics_reports_non_offercatch_service_on_runtime_port(self, check_node, ensure_runtime, runtime_health):
        from offercatch.browser.diagnostics import run_browser_diagnostics
        ...
        self.assertFalse(result["runtime"])
        self.assertTrue(any("non-OfferCatch" in error for error in result["errors"]))

    @patch("offercatch.browser.diagnostics.run_browser_diagnostics")
    def test_print_browser_diagnostics_shows_non_offercatch_service_message(self, run_browser_diagnostics):
        ...
        run_browser_diagnostics.return_value = {
            ...
            "targets": [],
            "boss_tab": None,
            "errors": ["Runtime port is occupied by a non-OfferCatch service."],
        ...
        self.assertIn("端口已被非 OfferCatch 服务占用", buffer.getvalue())
