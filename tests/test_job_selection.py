...
from offercatch.executor.sender import CHAT_BUTTON_SCRIPT_FOR_TESTS
from offercatch.executor.sender import _chat_target_matches_job
from offercatch.executor.sender import _confirm_preset_greeting
from offercatch.executor.sender import _handle_greet_popup
from offercatch.executor.sender import _message_delivery_state
from offercatch.executor.sender import _submitted_message_looks_accepted
from offercatch.executor.sender import send_greetings
from offercatch.executor.sender import _send_greeting_once
from offercatch.executor.sender import _submit_chat_message_background
from offercatch.executor.sender import _submit_startchat_greeting
from offercatch.executor.sender import _wait_for_chat_page
...
class JobSelectionTests(unittest.TestCase):
    ...
    def test_preset_confirmation_uses_background_dom_click(self):
        with patch(
            "offercatch.executor.sender.evaluate",
            return_value='{"success": true, "action": "preset_confirmed"}',
        ) as evaluate_mock, patch("offercatch.executor.sender.click_at") as click_at:
    ...
    def test_startchat_submission_uses_trusted_typing_and_real_send_message_click(self):
        ...
        with patch(
            ...
            "offercatch.executor.sender.click_at",
            return_value=True,
        ) as click_at, patch(
            "offercatch.executor.sender.press_key",
            return_value=True,
        ) as press_key, patch(
            "offercatch.executor.sender.type_text",
            return_value=True,
        ) as type_text:
    ...
    def test_startchat_popup_reuses_chat_redirect_without_foreground_input(self):
        ...
        with patch(
            "offercatch.executor.sender._detect_greet_popup",
            return_value={"success": True, "popup": True, "kind": "startchat_dialog"},
        ), patch(
            "offercatch.executor.sender._navigate_to_chat_redirect",
            return_value=True,
        ) as redirect, patch(
            "offercatch.executor.sender._submit_startchat_greeting",
    ...
    def test_send_greeting_reports_unavailable_job_page_before_clicking_chat(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1") as new_tab, \
             patch("offercatch.executor.sender.evaluate", return_value='{"success": false, "error": "job_page_unavailable", "history_detail": "岗位页面不存在或已下架", "skip_backoff": true}'), \
             patch("offercatch.executor.sender.close_tab") as close_tab, \
             patch("offercatch.executor.sender.time.sleep"):
    ...
    def test_send_greeting_uses_real_click_fallback_when_chat_button_does_not_navigate(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             patch("offercatch.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("offercatch.executor.sender._click_chat_button", return_value={"success": True, "button_text": "继续沟通"}), \
             patch("offercatch.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("offercatch.executor.sender._wait_for_chat_page", side_effect=[
                 {"success": False, "error": "chat_navigation_timeout"},
                 {"success": True, "target_id": "target-1"},
             ]), \
             patch("offercatch.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("offercatch.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("offercatch.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("offercatch.executor.sender.click_at", return_value=True) as click_at, \
             patch("offercatch.executor.sender.close_tab") as close_tab, \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_waits_for_chat_button_before_failing(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             patch("offercatch.executor.sender.evaluate", side_effect=evaluate_results) as evaluate_mock, \
             patch("offercatch.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("offercatch.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("offercatch.executor.sender._message_delivery_state", side_effect=["missing", "delivered", "delivered"]), \
             patch("offercatch.executor.sender._submit_chat_message_background", return_value={"success": False}), \
             patch("offercatch.executor.sender._fill_chat_input", return_value={"success": True, "disabled": False}), \
             patch("offercatch.executor.sender.click_at", return_value=True), \
             patch("offercatch.executor.sender.close_tab") as close_tab, \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_accepts_cleared_input_when_echo_is_missing(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             patch("offercatch.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("offercatch.executor.sender._click_chat_button", return_value={"success": True}), \
             patch("offercatch.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch("offercatch.executor.sender._wait_for_chat_page", return_value={"success": True, "target_id": "target-1"}), \
             patch("offercatch.executor.sender._message_delivery_state", return_value="missing"), \
             patch("offercatch.executor.sender._submit_chat_message_background", return_value={"success": True}), \
             patch("offercatch.executor.sender._submitted_message_looks_accepted", return_value=True), \
             patch("offercatch.executor.sender.close_tab") as close_tab, \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_send_greeting_uses_original_flow_when_platform_preset_is_enabled(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch(
                 "offercatch.executor.sender._confirm_preset_greeting",
                 return_value={"success": True, "action": "preset_confirmed"},
             ) as confirm_preset, \
             patch(
                 "offercatch.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ), \
             patch(
                 "offercatch.executor.sender._message_delivery_state",
                 side_effect=["missing", "delivered", "delivered"],
             ), \
             patch(
                 "offercatch.executor.sender._fill_chat_input",
                 return_value={"success": True, "disabled": False},
             ) as fill_input, \
             patch(
                 "offercatch.executor.sender._submit_chat_message_background",
                 return_value={"success": True, "action": "chat_submitted_background"},
             ) as background_submit, \
             patch("offercatch.executor.sender.click_at", return_value=True), \
             patch("offercatch.executor.sender.close_tab"), \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
                "您好，我对这个岗位很感兴趣。",
    ...
    def test_send_greeting_redirects_first_contact_and_uses_background_chat_submit(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             patch("offercatch.executor.sender.evaluate", return_value='{"success": true}'), \
             patch(
                 "offercatch.executor.sender._click_chat_button",
                 return_value={
                     "success": True,
             ...
             patch("offercatch.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch(
                 "offercatch.executor.sender._wait_for_chat_page",
                 return_value={"success": True, "target_id": "target-1"},
             ), \
             ...
             patch("offercatch.executor.sender._fill_chat_input") as fill_input, \
             patch("offercatch.executor.sender.close_tab") as close_tab, \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
    ...
    def test_first_contact_without_chat_redirect_fails_without_foreground_click(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="target-1"), \
             ...
             patch("offercatch.executor.sender._submit_startchat_greeting") as foreground_submit, \
             patch("offercatch.executor.sender.click_at") as foreground_click, \
             patch("offercatch.executor.sender.time.sleep"):
            result, target_id = _send_greeting_once(
                job,
                "您好，我对这个岗位很感兴趣。",
    ...
    def test_wait_for_chat_page_rejects_a_different_job_conversation(self):
        ...
        with patch("offercatch.executor.sender.evaluate", return_value="/web/geek/chat"), \
             patch("offercatch.executor.sender._chat_target_matches_job", return_value=False) as matches_job, \
             patch("offercatch.executor.sender.get_page_targets", return_value=[]), \
             patch("offercatch.executor.sender.time.sleep"):
    ...
    def test_wait_for_chat_page_adopts_preexisting_chat_when_active_job_matches(self):
        ...
        with patch("offercatch.executor.sender.evaluate", return_value="/job_detail/expected-job"), \
             patch("offercatch.executor.sender._chat_target_matches_job", return_value=True) as matches_job, \
             patch("offercatch.executor.sender.get_page_targets", return_value=targets), \
             patch("offercatch.executor.sender.time.sleep"):
    ...
    def test_send_greeting_adopts_matching_new_chat_tab_and_closes_old_tab(self):
        ...
        with patch("offercatch.executor.sender.new_tab", return_value="job-target"), \
             patch("offercatch.executor.sender.evaluate", return_value='{"success": true}'), \
             patch("offercatch.executor.sender._click_chat_button", return_value={"success": True}), \
             patch("offercatch.executor.sender._detect_greet_popup", return_value={"success": True, "popup": False}), \
             patch(
    ...
    def test_send_greetings_reopens_job_page_once_when_chat_input_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "offercatch.db"
            db = get_db(db_path)
            try:
                insert_job(db, job)
            ...
            with patch("offercatch.db.DB_PATH", db_path), \
                 patch("offercatch.executor.sender.should_take_day_off", return_value=False), \
                 patch("offercatch.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch("offercatch.executor.sender._send_greeting_once", side_effect=attempts) as send_once, \
                 patch("offercatch.executor.sender.close_tab") as close_tab:
    ...
    def test_send_greetings_closes_failed_task_page_after_recording_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("offercatch.db.DB_PATH", db_path), \
                 ...
                 patch("offercatch.executor.sender.SendWindowChecker.is_active", return_value=True), \
                 patch(
                     "offercatch.executor.sender._send_greeting_once",
    ...
    def test_pending_confirmation_excludes_jobs_with_greetings(self):
        with tempfile.TemporaryDirectory() as tmp:
            db = get_db(Path(tmp) / "offercatch.db")
            try:
    ...
    def test_send_greetings_force_bypasses_send_window_restriction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Act
            with patch("offercatch.db.DB_PATH", db_path), \
                 patch("offercatch.throttle.datetime") as mock_datetime, \
                 patch("offercatch.executor.sender._send_greeting_once", return_value=({"success": True}, None)):
