...
from offercatch.web.server import _execute_collect
from offercatch.web.tasks import WorkbenchTask
...
class ScraperBackgroundTests(unittest.TestCase):
    def test_stopped_collection_does_not_open_a_search_page(self):
        ...
        with patch("offercatch.scraper.jobs.get_db", return_value=db), \
             patch("offercatch.scraper.jobs.new_tab") as new_tab:
    ...
    def test_workbench_passes_its_stop_event_into_collection(self):
        ...
        with patch("offercatch.scraper.jobs.scrape_jobs", side_effect=scrape_with_progress) as scrape, \
             patch("offercatch.ai.scorer.score_jobs", side_effect=score_with_progress):
    ...
    def test_scraper_reports_seen_new_and_duplicate_counts(self):
        ...
        with patch("offercatch.scraper.jobs.get_db", return_value=db), \
             patch("offercatch.scraper.jobs.Progress", return_value=progress_context), \
             patch("offercatch.scraper.jobs.PageThrottle") as throttle_cls, \
             patch("offercatch.scraper.jobs.new_tab", side_effect=["search-target", "detail-target"]), \
             patch("offercatch.scraper.jobs.evaluate", side_effect=[json.dumps(jobs), json.dumps(detail)]), \
             patch("offercatch.scraper.jobs.wait_for_load"), \
             patch("offercatch.scraper.jobs.scroll"), \
             patch("offercatch.scraper.jobs.close_tab"), \
             patch("offercatch.scraper.jobs.job_exists", side_effect=[True, False]), \
             patch("offercatch.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("offercatch.scraper.jobs.insert_job"), \
             patch("offercatch.scraper.jobs.time.sleep"):
    ...
    def test_search_and_detail_pages_open_in_background(self):
        ...
        with patch("offercatch.scraper.jobs.get_db", return_value=db), \
             ...
             patch("offercatch.scraper.jobs.PageThrottle") as throttle_cls, \
             patch(
                 "offercatch.scraper.jobs.new_tab",
                 side_effect=["search-target", "detail-target"],
             ) as new_tab, \
             patch(
                 "offercatch.scraper.jobs.evaluate",
                 side_effect=[json.dumps(jobs), json.dumps(detail)],
             ), \
             patch("offercatch.scraper.jobs.wait_for_load"), \
             patch("offercatch.scraper.jobs.scroll"), \
             patch("offercatch.scraper.jobs.close_tab"), \
             patch("offercatch.scraper.jobs.job_exists", return_value=False), \
             patch("offercatch.scraper.jobs.matching_deal_breaker", return_value=False), \
             patch("offercatch.scraper.jobs.insert_job"), \
             patch("offercatch.scraper.jobs.time.sleep"):
            throttle_cls.return_value.wait.return_value = None
