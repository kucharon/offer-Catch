...
def test_scoring_never_selects_soft_deleted_pending_jobs(tmp_path):
    ...
    with patch("offercatch.ai.scorer.get_db", side_effect=lambda: get_db(db_path)), \
         patch("offercatch.ai.scorer._load_resume", return_value="resume"), \
         patch("offercatch.ai.scorer._request_score") as request_score:
