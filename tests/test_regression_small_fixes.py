...
class VersionMetadataTests(unittest.TestCase):
    def test_release_version_is_consistent(self):
        ...
        import offercatch
        from offercatch.web.server import health
        ...
        self.assertEqual(offercatch.__version__, "2.3.0")
...
class ConfigValidationTests(unittest.TestCase):
    ...
    def test_monitor_does_not_follow_up_when_setting_is_missing(self):
        from offercatch.executor import monitor
...
class AiPromptRegressionTests(unittest.TestCase):
    def test_scorer_prompt_uses_universal_weighted_evidence(self):
        from offercatch.ai.scorer import SCORING_PROMPT
...
class PrefilterHardGateTests(unittest.TestCase):
    def test_anonymous_company_jobs_are_filtered_before_ai_scoring(self):
        from offercatch.ai.prefilter import quick_score
...
class ConfirmationUiTests(unittest.TestCase):
    @patch("offercatch.ui.confirm.Prompt.ask")
    @patch("offercatch.ui.confirm.get_jobs_pending_confirmation")
    @patch("offercatch.ui.confirm.get_db")
...
class ConfigSchemaTests(unittest.TestCase):
    def setUp(self):
        ...
        self.schema_source = (
            ROOT / "src" / "offercatch" / "web" / "config_schema.json"
        ).read_text(encoding="utf-8")
...
class ScorerPrefilterTests(unittest.TestCase):
    def setUp(self):
        self.source = (ROOT / "src" / "offercatch" / "ai" / "scorer.py").read_text(encoding="utf-8")
