...
from offercatch.ai.scorer import ScoreOutcome, score_jobs
...
def test_pause_checkpoint_keeps_current_and_unstarted_jobs_for_recovery(tmp_path):
	...
	with (
		patch("offercatch.ai.scorer.get_db", side_effect=lambda: get_db(db_path)),
		patch("offercatch.ai.scorer._load_resume", return_value="real resume"),
		patch("offercatch.ai.scorer.quick_score", return_value=(80, "pass")),
		patch("offercatch.ai.scorer._score_job_with_ai", side_effect=outcomes),
	):
		score_jobs(
			{
