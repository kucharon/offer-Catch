...
from offercatch.db import add_history, get_db, insert_job, update_job_status
from offercatch.throttle import RequestThrottle
...
class MonitorThrottleTests(unittest.TestCase):
    def test_mark_makes_configured_request_interval_effective(self):
        ...
        with patch("offercatch.throttle.time.time", return_value=100), \
             patch("offercatch.throttle.random.gauss", return_value=60), \
             patch("offercatch.throttle.random.random", return_value=1), \
             patch("offercatch.throttle.time.sleep") as sleep:
            throttle.mark()
...
class MonitorIdempotencyAndLimitTests(unittest.TestCase):
    def test_same_unresolved_reply_is_skipped_but_new_hr_message_is_processed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "data" / "offercatch.db"
            db = get_db(db_path)
            try:
...
class FullFlowMonitorCooldownTests(unittest.TestCase):
    def test_full_flow_initial_cooldown_is_cancellable_before_first_scan(self):
        from offercatch.web.tasks import WorkbenchTask, wait_for_initial_monitor_cooldown
