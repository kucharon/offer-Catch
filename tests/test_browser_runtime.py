...
class BrowserRuntimeManagerTests(unittest.TestCase):
    def test_get_runtime_url_uses_configured_host_and_port(self):
        from offercatch.browser.runtime import get_runtime_url
    ...
    def test_runtime_script_path_points_to_bundled_proxy(self):
        from offercatch.browser.runtime import get_runtime_script_path
    ...
    @patch("offercatch.browser.runtime.subprocess.run")
    @patch("offercatch.browser.runtime._candidate_node_executables")
    def test_check_node_available_returns_version(self, candidates, run):
        from offercatch.browser.runtime import check_node_available
    ...
    @patch("offercatch.browser.runtime.subprocess.run")
    @patch("offercatch.browser.runtime.Path.is_file", return_value=True)
    @patch("offercatch.browser.runtime._candidate_node_executables")
    ...
    @patch("offercatch.browser.runtime.httpx.get")
    def test_runtime_targets_returns_array_from_offercatch_runtime(self, http_get):
        from offercatch.browser.runtime import runtime_targets

        health = Mock(status_code=200)
        health.json.return_value = {"status": "ok", "runtime": "offercatch"}
    ...
    @patch("offercatch.browser.runtime.httpx.get")
    def test_runtime_targets_rejects_non_offercatch_runtime(self, http_get):
        from offercatch.browser.runtime import runtime_targets
    ...
    @patch("offercatch.browser.runtime.runtime_targets")
    @patch("offercatch.browser.runtime.start_runtime")
    @patch("offercatch.browser.runtime.check_node_available")
    def test_ensure_runtime_starts_builtin_runtime_when_auto_start_enabled(self, check_node, start_runtime, runtime_targets):
        from offercatch.browser.runtime import ensure_runtime
    ...
    @patch("offercatch.browser.runtime.time.sleep")
    @patch("offercatch.browser.runtime._is_port_available")
    @patch("offercatch.browser.runtime.runtime_health")
    @patch("offercatch.browser.runtime.runtime_targets")
    @patch("offercatch.browser.runtime.start_runtime")
    @patch("offercatch.browser.runtime.check_node_available")
    def test_ensure_runtime_uses_next_free_port_when_default_has_non_offercatch_service(
        self,
        check_node,
        start_runtime,
        runtime_targets,
        ...
        from offercatch.browser.runtime import ensure_runtime, get_runtime_url
    ...
    @patch("offercatch.browser.runtime.subprocess.Popen")
    def test_start_runtime_sets_offercatch_environment(self, popen):
        from offercatch.browser.runtime import start_runtime

        with patch.dict(os.environ, {}, clear=True):
            start_runtime({"browser": {"proxy_port": 4567, "chrome_ports": [9222, 9333], "enable_port_guard": False}})

        env = popen.call_args.kwargs["env"]
        self.assertEqual(env["OFFERCATCH_BROWSER_PROXY_PORT"], "4567")
        self.assertEqual(env["OFFERCATCH_CHROME_PORTS"], "9222,9333")
        self.assertEqual(env["OFFERCATCH_ENABLE_PORT_GUARD"], "false")
...
class BrowserRuntimeSourceTests(unittest.TestCase):
    def test_cdp_proxy_fetches_browser_websocket_url_for_fallback_ports(self):
        script = Path(__file__).parents[1] / "src" / "offercatch" / "browser" / "runtime" / "cdp-proxy.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("/json/version", source)
        self.assertIn("webSocketDebuggerUrl", source)

    def test_cdp_proxy_only_reuses_offercatch_runtime_on_occupied_port(self):
        script = Path(__file__).parents[1] / "src" / "offercatch" / "browser" / "runtime" / "cdp-proxy.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("const RUNTIME_NAME = 'offercatch'", source)
    ...
    def test_check_runtime_requires_offercatch_runtime_identity(self):
        script = Path(__file__).parents[1] / "src" / "offercatch" / "browser" / "runtime" / "check-runtime.mjs"
        source = script.read_text(encoding="utf-8")

        self.assertIn("/health", source)
        self.assertIn("runtime === 'offercatch'", source)
